...
<meta charset="utf-8">
<title>无标题文档</title>
<link href="__PUBLIC__/css/search.css" rel="stylesheet" media="screen" type="text/css" />
...
<div id="search_main">
       <div class="top">
       <div id="search_top">
       <a href="__ROOT__">首页</a> <code>&gt;</code> 商品搜索_<?php echo ($_GET['keywords']); ?>
       </div>
       <div id="search_way">
       <form action="__APP__/Search/sort" method="get">
	   排序方式：
	   <select name="sort">
	   <option value="Goods_SellTime">按上架时间排序</option>
       <option value="Goods_SellNum" selected>按销售量排序</option>
       <option value="Goods_Price">按价格排序</option>
	   <input id="img" type="image" name="imageField" src="__PUBLIC__/images/bnt_go.gif" alt="go"/>				
       </form>
       </div>
       </div>
       <div id="good_list">
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="good_single">
				<a href="__APP__/GoodsInfo/index/goodsid/<?php echo ($vo["pk_GoodsInfo_Id"]); ?>"><img src="<?php echo ($vo["Goods_img"]); ?>" width="175" height="175"></a>
		        <a href="__APP__/GoodsInfo/index/goodsid/<?php echo ($vo["pk_GoodsInfo_Id"]); ?>" class="good_name"><?php echo ($vo["Goods_Name"]); ?></a>
		        <span class="good_price"><?php echo ($vo["Goods_Price"]); ?></span>

		        <p class="buy_panel"><a href="__APP__/Cart/add" class="buy">购买</a></p>
